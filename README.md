...
This plot uses `ecorys_mono_pal()` inside of `scale_fill_manual()` to
generate a monochrome blue palette of the same length as the
...
diamonds %>% 
  ...
  scale_fill_manual(values = ecorys_mono_pal(n_distinct(diamonds$clarity)),
                    name = "Clarity") +
